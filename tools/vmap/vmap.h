...
typedef struct metaTriangle_s
{
	shaderInfo_t        *si;
	side_t              *side;
	int entityNum, surfaceNum, planeNum, fogNum, cubemapNum, sampleSize, castShadows, recvShadows;
	float shadeAngleDegrees;
	vec4_t plane;
	vec3_t lightmapAxis;
	int indexes[ 3 ];
...
int                         CubemapForBounds( vec3_t mins, vec3_t maxs );
void                        CreateMapCubemaps( void );
...
void                        RadLightForPatch( int num, int lightmapNum, rawLightmap_t *lm, shaderInfo_t *si, float scale, float subdivide, clipWork_t *cw, qboolean fixed );
...
void BSPX_Setup(void *filebase, size_t filelen, bspLump_t *lumps, size_t numlumps);
...
;
	...
	=
	{
	#include "game_fte.h"  /* spike: fte engine */
	,
...
Q_EXTERN qboolean externalHDRLightmaps Q_ASSIGN( qfalse );
...
Q_EXTERN bspx_t *bspx Q_ASSIGN( NULL );
...
Q_EXTERN int numBSPCubemaps Q_ASSIGN( 0 );
Q_EXTERN denvmap_t bspCubemaps[ 0x200 ];
Q_EXTERN int   *bspDrawSurfaceCubemaps Q_ASSIGN( NULL );
